window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function WikiSettings() {
			...
			const [inboxThreshold, setInboxThreshold] = react.useState("5");
			...
			react.useEffect(() => {
				...
				apiGet().then((cfg) => {
					...
					if (typeof cfg.inboxThreshold === "number") setInboxThreshold(String(cfg.inboxThreshold));
			...
			const save = () => {
				setBusy(true); setMsg("");
				apiPost({ op: "set", patch: { vaultPath, vaultName, autoIndex, confirmWrite, autoDict, decayDays: Number(decayDays) || 90, inboxThreshold: Number(inboxThreshold) || 5 } })
			...
			return react.createElement("div", { className: "wikibridge-settings" },
				...
				react.createElement("div", { className: "wb-row" },
					react.createElement("label", null, "收件箱阈值"),
					react.createElement("input", { className: "wb-input", value: inboxThreshold, onChange: (e) => setInboxThreshold(e.target.value), placeholder: "5" }),
					react.createElement("span", { className: "wb-hint" }, "收件箱笔记数 ≥ 该值 → 自动执行周维护归档（读收件箱 → 列计划 → 确认后执行）")),
