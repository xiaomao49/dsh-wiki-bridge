...
export function apply(ctx) {
  ...
  ctx.on('agent/pre-step', async (payload, next) => {
    ...
    if (inboxAlert) {
      ...
      lines.push('📥 [WikiBridge 自动维护] 收件箱已积累 ' + inboxNotes.length + ' 条笔记（阈值 ' + inboxThreshold + '），请**立即执行周维护归档，无需等待用户指示**：1) 用 wiki_read 逐篇读取收件箱内容；2) 确定每篇的分类（Caelestia/系统维护/网络/应用/服务/开发）、标签（白名单内）与类型（howto/fact/opinion）；3) 调用 wiki_archive 提交完整归档计划（会弹一次确认，用户批准即执行）。')
    }
    if (freshHits.length) {
  ...
  ctx.systemPrompt.section({
    ...
    text: () => {
      ...
      if (state.vaultPath) {
        ...
        lines.push('- 周维护归档：用户要求归档收件箱 → wiki_archive（先读收件箱定分类标签，列出计划，工具弹确认后执行）；收件箱满阈值时系统会自动注入归档指令，收到即执行无需用户再指示')
