...
export function apply(ctx) {
  ...
  const CONFIG_FIELDS = ['vaultPath', 'vaultName', 'autoIndex', 'confirmWrite', 'autoDict', 'decayDays']
  ...
  // pre-step 瀑布：仅 step 1（新用户消息进入）且命中主题词时注入；
  // 每回合只一次；同一笔记在最近 RECENT_TURNS 回合内注入过则跳过（短窗口去重）
  ctx.on('agent/pre-step', async (payload, next) => {
    ...
    if (payload.step !== 1 || (recent && recent.turn === payload.turn)) return decision
    ...
    // 过滤最近窗口内已注入的笔记；全部重复则不注入
    const withinWindow = recent && payload.turn - recent.turn <= RECENT_TURNS
    const freshHits = withinWindow ? hits.filter((n) => !recent.paths.has(n.path)) : hits
    if (!freshHits.length) return decision
    const paths = withinWindow ? new Set(recent.paths) : new Set()
  ...
  async function executeWrite(args, exec) {
    ...
    return { ok: true, action, path: relPath, title, tags, date, redacted: redacted.count, redactedCount: redacted.count }
  }
  ...
  const writeSchema = out({ type: 'object', additionalProperties: false, properties: { ok: { type: 'boolean' }, reason: str, action: str, path: str, title: str, tags: { type: 'array', items: str }, date: str, redacted: { type: 'number' }, redactedCount: { type: 'number' } } })
