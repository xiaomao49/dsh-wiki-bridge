...
const isMeta = (rel) => META_FILES.includes(rel)
...
export function apply(ctx) {
  ...
  const CONFIG_FIELDS = ['vaultPath', 'vaultName', 'autoIndex', 'confirmWrite', 'autoDict']
  ...
  let indexCache = null
  let msgSeq = 0
  const lastInjectedTurn = new Map()
  ...
  function parseFrontmatter(text) {
    const meta = { title: null, tags: [], created: null, updated: null }
  ...
  function buildFrontmatter(title, tags, created, updated) {
    ...
    return ['---', 'title: ' + title, 'created: ' + created, 'updated: ' + updated, 'tags: [' + tagLine + ']', '---', ''].join('\n')
  }
  ...
  // pre-step 瀑布：仅 step 1（新用户消息进入）且命中主题词时注入；每 agent 每回合一次
  ctx.on('agent/pre-step', async (payload, next) => {
    const decision = await next()
    if (!state.autoDict) return decision
    if (decision.kind !== 'enter') return decision
    const agentId = payload.agent ? payload.agent.id : '?'
    if (payload.step !== 1 || lastInjectedTurn.get(agentId) === payload.turn) return decision
    const text = decision.messages.map((m) => (m.content || []).map((b) => (typeof b === 'object' && b ? b.text || '' : '')).join(' ')).join('\n').slice(0, 800)
    if (!text.trim()) return decision
    let hits
    try { hits = matchDict(text, await ensureIndex(payload.signal)) } catch (e) {
      console.log('[wiki-bridge] 词典检索跳过：', String(e && e.message || e))
      return decision
    }
    if (!hits || !hits.length) return decision
    lastInjectedTurn.set(agentId, payload.turn)
    const lines = ['[WikiBridge 知识库检索] 对话主题与以下笔记相关（需要时用 wiki_read 读取全文）：']
    for (const n of hits) {
      lines.push('- ' + n.path + '｜' + n.title + '｜' + String(n.snippet).replace(/\n/g, ' '))
    }
  ...
  async function executeArchive(args, exec) {
    ...
    for (const it of items) {
      ...
      plan.push({
        ...
        meta, body, desc: String(it.desc || '').trim(),
      })
    }

    // 一次确认全部计划
    if (state.confirmWrite) {
      const detail = plan.map((p) => '- ' + p.srcRel + ' → ' + p.destRel + (p.desc ? '（' + p.desc + '）' : '')).join('\n')
    ...
    for (const p of plan) {
      try {
        ...
        const newText = buildFrontmatter(p.newTitle, p.tags, p.meta.created || date, date) + newBody.replace(/^\n+/, '') + '\n'
  ...
  async function executeWrite(args, exec) {
    ...
    if (!content) throw new Error('content 不能为空（只写提炼后的知识，不写对话原文）')
    ...
    if (action === 'created') {
      ...
      if (!ok) return { ok: false, reason: 'cancelled' }
      const text = buildFrontmatter(title, tags, date, date) + content + '\n'
      ...
      const text = buildFrontmatter(meta.title || title, meta.tags.length ? meta.tags : tags, meta.created || date, date) + newBody
      const ok = await confirmWrite(exec, '追加更新 ' + relPath, '已有页面（updated 将更新为 ' + date + '）\n\n' + content.slice(0, 200), userConfirmed)
      if (!ok) return { ok: false, reason: 'cancelled' }
      await ctx.fs.writeText(writeTarget, text, undefined, exec.signal)
    }

    indexCache = null // 写入后索引失效，下次检索重建
    return { ok: true, action, path: relPath, title, tags, date }
  }
  ...
  const writeSchema = out({ type: 'object', additionalProperties: false, properties: { ok: { type: 'boolean' }, reason: str, action: str, path: str, title: str, tags: { type: 'array', items: str }, date: str } })
  ...
  const defs = [
    ...
    defineTool({
      name: 'wiki_write',
      description: '把提炼后的知识写入 Obsidian 知识库。遵守 _约定.md：自动生成 frontmatter（title/created/updated/tags）、文件名用中文短名、默认写入 收件箱/（铁律：平时不写其他目录）、发现同名页面时询问追加更新而非重复建页、新标签需用户确认。content 必须是提炼后的知识，不是对话原文。date 可选，默认当天日期。用户明确要求"记一下/存进知识库"时传 confirmed=true 跳过写入确认弹窗（用户已口头授权）；模型主动沉淀时必须 confirmed=false 走确认。',
      parameters: {
    ...
    defineTool({
      name: 'wiki_archive',
      description: '每周归档：把收件箱笔记移入分类目录并登记首页。遵守 _约定.md：源必须在 收件箱/、目标必须是合法分类（Caelestia/系统维护/网络/应用/服务/开发）、标签必填且在白名单内、自动刷新 frontmatter 的 updated 与正文标题、自动登记首页。调用前先用 wiki_search/wiki_read 查看收件箱内容，确定每篇的分类与标签后列出完整计划 items；工具会先弹一次确认再执行。用户说"做周维护/归档收件箱"时使用。',
      parameters: {
        items: {
